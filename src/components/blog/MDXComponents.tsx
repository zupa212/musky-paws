...
const KeyTakeaways = ({ children }: MDXProps) => (
    <div className="bg-brand-50 border border-brand-accent-pink/30 rounded-2xl p-6 my-8 shadow-sm">
        <h3 className="flex items-center gap-2 text-lg font-bold text-navy-900 mb-4 mt-0">
            <CheckCircle2 className="w-5 h-5 text-brand-accent-pink" />
            Key Takeaways
        </h3>
    ...
    return (
        <details className="group border border-brand-200 bg-white rounded-2xl overflow-hidden shadow-sm">
            <summary className="flex items-center justify-between cursor-pointer p-5 font-semibold text-navy-900 list-none [&::-webkit-details-marker]:hidden">
                {q}
                <ChevronDown className="w-5 h-5 text-brand-accent-pink transition-transform group-open:rotate-180" />
            </summary>
...
const CallToAction = ({ primaryText, primaryLink, secondaryText, secondaryLink }: any) => (
    <div className="bg-navy-900 rounded-[32px] p-8 md:p-12 text-center my-12 shadow-xl">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link href={primaryLink || '/booking'} className="w-full sm:w-auto bg-brand-accent-pink text-navy-900 px-8 py-4 rounded-full font-bold hover:scale-105 transition-transform flex items-center justify-center">
...
export const MDXComponents = {
    ...
    ul: (props: any) => <ul className="list-disc list-outside ml-6 text-navy-900/70 mb-6 space-y-2 marker:text-brand-accent-pink" {...props} />,
    ol: (props: any) => <ol className="list-decimal list-outside ml-6 text-navy-900/70 mb-6 space-y-2 marker:text-brand-accent-pink" {...props} />,
    li: (props: any) => <li className="pl-2 leading-relaxed" {...props} />,
    a: (props: any) => <a className="text-brand-accent-pink font-semibold hover:underline underline-offset-4 decoration-2" {...props} />,
    strong: (props: any) => <strong className="font-bold text-navy-900" {...props} />,
    blockquote: (props: any) => <blockquote className="border-l-4 border-brand-accent-pink pl-6 py-2 my-8 italic text-navy-900/60 bg-brand-50 rounded-r-xl" {...props} />,
    img: (props: any) => (
